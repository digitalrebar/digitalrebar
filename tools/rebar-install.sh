...
cd /etc/yum.repos.d
# Setup OCB repo
cat > rebar-$RELEASE-install.repo <<EOF
[rebar-$RELEASE]
name=$RELEASE repo for digitalrebar rpms
...
if [[ $1 = '--without-hardware' ]]; then
    shift
    yum install -y digitalrebar-core
else
    yum install -y digitalrebar-hardware
    WITH_HARDWARE="YES"
fi
...
if [[ $WITH_HARDWARE = 'YES' ]]; then
    ...
    echo "See: https://github.com/digitalrebar/hardware/tree/master/doc"
    echo ""
fi
...
if [[ $WITH_DOWNLOAD = 'NO' ]]; then
  ...
  echo "See: https://github.com/digitalrebar/core/tree/master/doc/deployment-guide/adding-operating-systems.md"
fi
