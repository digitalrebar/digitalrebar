...
for stage in boot consul database chef-server core ; do
    "./rebar-${stage}.sh" 2>&1 |tee "/var/log/rebar/install-${stage}.log"
done

# Clean up keys and things
rm -rf /var/log/rebar/install-*
rm -f /etc/profile.d/rebar-key.sh
rm -f /etc/rebar.install.key
rm -rf /root/.ssh/*
rm -rf /home/rebar/.ssh/id*
rm -rf /root/rebar.pem
# Clean up goiardi
rm -rf /etc/goiardi/*
# Clean up database
