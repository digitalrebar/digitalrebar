...
bash "Create DNS #{server_name} Management ACL" do
  code <<EOC
    echo '{
      ...
      "Type": "client",
      "Rules": "key \\"opencrowbar/private\\" { policy = \\"deny\\" }\\nkey \\"opencrowbar/private/dns-mgmt/#{server_name}\\" { policy = \\"write\\" }"
    }' > /tmp/tmp_cred.json
...
bash "Add dns mgmt #{server_name} keys to consul" do
  code <<EOC
    curl -X PUT -d '#{access_name}' http://127.0.0.1:8500/v1/kv/opencrowbar/private/dns-mgmt/#{server_name}/access_name?token=#{master_acl_token}
    curl -X PUT -d '#{access_password}' http://127.0.0.1:8500/v1/kv/opencrowbar/private/dns-mgmt/#{server_name}/access_password?token=#{master_acl_token}
    curl -X PUT --data-binary @/etc/dns-mgmt-https-cert.pem http://127.0.0.1:8500/v1/kv/opencrowbar/private/dns-mgmt/#{server_name}/cert_pem?token=#{master_acl_token}
EOC
...
template '/etc/consul.d/crowbar-dns-mgmt-server.json' do
