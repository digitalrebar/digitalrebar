...
node["rebar"]["proxy"]["networks"].each do |r|
  localnets << r
end
localnets = localnets.uniq.sort!

proxy_port = (node["rebar"]["proxy"]["server"]["port"] rescue 8123) || 8123

upstream_proxy = (node["rebar"]["proxy"]["server"]["upstream_proxy"] rescue nil)
...
template "/etc/consul.d/rebar-squid.json" do
