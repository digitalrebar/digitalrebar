...
inspector(_Config, F) -> bdd_utils:depricate({2013, 8, 1}, rebar_rest, inspector, rebar_rest, inspector, [F]).
inspector(Feature) ->
  R = eurl:get_http(rebar_rest:alias(Feature, g, [path])),
...
step(_Given, {step_when, {_Scenario, _N}, ["REST gets the",barclamp,Barclamp,Resource,"list"]}) -> 
  Path = eurl:path([api,rebar:g(version),barclamps,Barclamp,bdd_restrat:alias(Resource,g,[resource])]),
  bdd_utils:log(debug, rebar, step, "REST get ~p list for ~p barclamp", [Resource, Barclamp]),
  eurl:get_http(Path);

% ============================  THEN STEPS =========================================

% validate object based on basic rules for Rebar
...
% ============================  LAST RESORT =========================================
step(_Given, {step_when, _N, ["I have a test that is not in Rebar_Rest"]}) -> true;
