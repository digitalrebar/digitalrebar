...
g(Item) ->
  case Item of
    ...
    blocked -> 3;
    proposed -> 4;
    _ -> bdd_utils:log(warn, rebar, g, "Could not resolve g request for ~p (fall through catch)", [Item]), false
  end.
...
i18n_lookup(T) -> 
  ...
  bdd_utils:log(trace, rebar, i18n, "looking up ~p", [URI]),
  R = eurl:get_http(URI),
  case R#http.code of
    200 -> R#http.data;
    _   -> bdd_utils:log(warn, rebar, i18n, "Translation for ~p not found", [URI]), "!TRANSLATION MISSING!"
  end.

% rest response specific for rebar API (only called when the vnd=rebar)
parse_object(Results) ->
  [Type, Quantity | _] = Results#http.details,
  case Quantity of 
    "obj"   ->  JSON = json:parse(Results#http.data),
                ID = proplists:get_value("id", JSON),
                #obj{namespace = rebar, data=JSON, type = Type, id = ID, url = Results#http.url};
    "array" ->  JSON = json:parse(Results#http.data),
                #array{namespace = rebar, data=JSON, type = Type, url = Results#http.url, count = length(JSON) };
    "result" -> JSON = json:parse(Results#http.data),
                #array{namespace = rebar, data=JSON, type = Type, url = Results#http.url, count = length(JSON) };
    "list"  ->  JSON = json:parse(Results#http.data),
                IDs = [proplists:get_value("id", I) || I <- JSON],
                #list{namespace = rebar, data=JSON, type = Type, ids = IDs, url = Results#http.url, count = length(IDs) };
    "empty" ->  #obj{namespace = rebar, data=none, type = Type, id = -1, url = Results#http.url };
    "error" ->  #obj{namespace = rebar, data=error, type = Type, id = -1, url = Results#http.url };
    _       ->  bdd_utils:log(warn, "Rebar API returned unexpected quantity flag (expected obj or list).  Returned ~p", [Results]),
                #item{namespace = rebar, data=Results#http.data , url=Results#http.url}
  end.
...
wait_for(R, Match) when is_number(Match) ->
  case R#http.code of
    Match ->  true;
    X     ->  bdd_utils:log(debug, rebar, wait_for, "Waiting on ~p to return ~p was ~p",[R#http.url, Match, X]), 
              false
  end;
wait_for(R, Match) ->
  case R#http.data of
    Match ->  true;
    X     ->  bdd_utils:log(debug, rebar, wait_for, "Waiting on ~p.  Result is ~p long",[R#http.url, length(X)]), 
              false
  end.

wait_for(URL, MatchCode, MatchData, 0, _) -> 
  bdd_utils:log(error, rebar,wait_for, "Did not get ~p/~p from ~p after repeats", [MatchCode, MatchData, URL]),
...
step(_Global, {step_setup, {_Scenario, _N}, Test}) -> 
  ...
  bdd_utils:log(debug, rebar, step, "Global Setup alias: ~p",[get({scenario,alias_map})]),
  ...
  bdd_utils:log(debug, rebar, global_setup, "Adding Service Roles", []),
  Phantom = bdd_utils:config(system_phantom,"system-phantom.internal.local"),
  PhantomRoles = bdd_utils:config(system_phantom_roles, ["dns-service", "ntp-service", "proxy-service", "provisioner-service", "dns-mgmt_service", "rebar-api_service", "rebar-job_runner_service"]),
  ServiceNRs = eurl:path([node:g(path), Phantom, "node_roles"]),
  R = eurl:get_http(ServiceNRs),
  O = bdd_restrat:get_object(R),
  bdd_utils:log(debug, rebar, global_setup, "Checking for Phantom Node Roles ~p",[O#list.count]),
  case O#list.count of
    1 ->  [node:bind(Phantom,PR) || PR <- PhantomRoles],
          bdd_utils:log(info, rebar, global_setup, "Bound Roles ~p to Phantom ~p",[PhantomRoles, Phantom]),
          node:commit(Phantom),
          node:alive(Phantom);
    _  -> noop 
  end,
  % create node for testing
  bdd_utils:log(debug, rebar, global_setup, "Global Setup running (creating node ~p)",[g(node_name)]),
  node:add_node(g(node_name), "rebar-admin-node", [{description, Test ++ g(description)}, {order, 100}, {admin, "true"}, {ip, "192.168.124.10/24"}], g(node_atom)),
  true;

% find the node from setup and remove it
step(_Global, {step_teardown, {_Scenario, _N}, _}) -> 
  bdd_utils:log(debug, rebar, step, "Global Teardown running",[]),
  % skip some activity if we're logging at debug level
  case lists:member(debug,get(log)) of
    true -> bdd_utils:log(debug, rebar, step, "Skipping Teardown Queue Empty & Admin Remove",[]);
    _ ->
      % clear quueue
...
step(Global, {step_given, {ScenarioID, _N}, ["there is a",role, Name]}) -> 
  step(Global, {step_given, {ScenarioID, _N}, ["there is a",role, Name, "in", barclamp, "rebar", "for", jig, "test"]});

step(_Global, {step_given, {ScenarioID, _N}, ["there is a",role, Name, "in", barclamp, Barclamp, "for", jig, Jig]}) -> 
  bdd_utils:log(debug, rebar, step, "REST creates the ~p ~p", [role, Name]),
...
step(_Given, {step_when, {ScenarioID, _N}, [deployment,Deployment,"includes",role,Role]}) -> 
  bdd_utils:log(debug, rebar, step, "REST addes role ~p to deployment ~p", [Role, Deployment]),
...
step(_Given, {step_when, _N, ["REST gets the",network,Network,range,"list"]})  -> 
  ...
  bdd_utils:log(debug, rebar, step, "REST range get ~p list for ~p path", [Network, URI]),
  R = eurl:get_http(URI),
...
step(_Given, {step_when, _N, ["REST gets the",network,Network,range,Key]})  ->
  ...
  bdd_utils:log(debug, rebar, step, "REST range get the object ~p for ~p path", [Network, URI]),
  bdd_restrat:step(_Given, {step_when, _N, ["REST requests the",URI,"page"]});

step(_Global, {_, {_Scenario, _N}, [node,Node,"has the",Role,"role"]}) -> 
  bdd_utils:log(debug, rebar, step, "REST add role ~p to node ~p", [Role, Node]),
  node:bind(Node, Role);

step(_Given, {_, {_Scenario, _N}, [node,Node,"is committed"]}) -> 
  bdd_utils:log(debug, rebar, step, "REST commits node ~p", [Node]),
  node:commit(Node),
  node:alive(Node);

step(_Given, {_, {_Scenario, _N}, ["REST retries",node,Node,node_role,Role]}) -> 
  bdd_utils:log(debug, rebar, step, "REST retry Node ~p Role ~p", [Node, Role]),
  Path = eurl:path([node:g(path),Node,"node_roles"]),
  Obj = bdd_crud:read_obj(Path,Role),
  bdd_utils:log(trace, rebar, step, "REST retry object ~p", [Obj]),
  R = eurl:path([node_role:g(path),Obj#obj.id,"retry"]),
  bdd_utils:log(debug, rebar, step, "REST retry path ~p", [R]),
  bdd_crud:update(R, "");
...
step(_Result, {step_then, {_Scenario, _N}, [node, Node, "should be in state", State]}) -> 
  ...
  bdd_utils:log(debug, rebar, step, "Node ~p checking for state ~p", [URI, State]),
  R = eurl:get_http(URI),
  {array, rebar, "json", J, _, _, _} = bdd_restrat:get_object(R),
  bdd_utils:log(trace, rebar, step, "Node ~p returned ~p", [Node, J]),  
  [{_ID,[{"name",Node},{"state",_},{"status",Status},_]}] = J,
  bdd_utils:log(debug, rebar, step, "Node ~p returned ~p", [Node, Status]),  
  Status =:= State;

% ============================  CLEANUP =============================================

step(_, {_, {_Scenario, _N}, ["there are no pending Rebar runs for",node,Node]}) -> 
