...
class BarclampRebar::AnsiblePlaybookJig < Jig
  ...
  def run(nr,data)
    ...
    unless File.exists?("#{cache_dir}/#{nr.role.name}")
      if role_yaml['playbook_src_path'] =~ /^http/
        # Load the git cache
        out, err, status = exec_cmd("git clone #{role_yaml['playbook_src_path']} #{cache_dir}/#{nr.role.name}")
        die "Failed to get @ #{role_file}: #{out} #{err}" unless status.success?
      else
        FileUtils.cp_r("#{local_scripts}/#{role_yaml['playbook_src_path']}/.", "#{cache_dir}/#{nr.role.name}")
      end
    else
      if role_yaml['playbook_src_path'] =~ /^http/
        # Update git repos??
    ...
    remote_tmpdir,err,ok = exec_cmd("mktemp -d /tmp/ansible-playbook-jig-XXXXXX")
    remote_tmpdir.strip!
    if remote_tmpdir.empty? || !ok.success?
      die "Did not create remote_tmpdir for some reason! (#{err})"
    end

    # Make a temp copy of playbook
    FileUtils.cp_r("#{cache_dir}/#{nr.role.name}", remote_tmpdir)

    rundir = "#{remote_tmpdir}/#{nr.role.name}/#{role_yaml['playbook_path']}"
    ...
    out,err,ok = exec_cmd("cd #{rundir} ; ansible-playbook -l #{nr.node.address.addr} -i inventory.ini --extra-vars \"@rebar.json\" #{role_yaml['playbook_file']}")
