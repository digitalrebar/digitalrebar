...
rebar nodes bind "$FQDN" to rebar-build-root-key
rebar nodes bind "$FQDN" to rebar-admin-node
...
ip_re='([0-9a-f.:]+/[0-9]+)'
netline=$(rebar nodes addresses "$FQDN" on $admin_net_name)
nets=(${netline//,/ })
for net in "${nets[@]}"; do
    [[ $net =~ $ip_re ]] || continue
    net=${BASH_REMATCH[1]}
...
if [[ $http_proxy ]] ; then
    ...
    (
        . bootstrap.sh
        chef-solo -c /opt/digitalrebar/core/bootstrap/chef-solo.rb -o "${proxy_recipes}"
    )
    . /etc/profile
    ...
    service rebar stop
    service rebar start
    while ! rebar users list; do
        sleep 1
    done
fi

rebar nodes commit "$FQDN"

# Mark the node as alive.
rebar nodes update "$FQDN" '{"alive": true}'
